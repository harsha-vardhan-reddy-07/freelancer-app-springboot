...
const Freelancer = () => {
  ...
  const updateUserData = async() =>{
    let skills =  updateSkills.split(',').map((skill) => skill.trim());
    axios.post(`http://localhost:6001/update-freelancer`, {userId: freelancerId, skills, description: updateDescription}).then(
        (response)=>{
          fetchUserData();
